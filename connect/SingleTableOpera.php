<?php
require_once dirname(__FILE__) . '/../model/SqlHelper.class.php';
/**
 * 对数据库单表的操作类 
...
class SingleTableOpera{
    ...
    **/
    ...
    function getObject(array $args = array(), $or = 0) {        
        $field = isset($args['_field']) ? $args['_field'] :  "*";
        ...
        */
        if($or) {
       		$where = isset($args['_where']) ? $args['_where'] : '0'; 	
        } else {
        	$where = isset($args['_where']) ? $args['_where'] : '1'; 	
        }
...
/*要保证传入的数组 的 索引 与 数据表的列名 一致*/       
   ...
   $str .= "INSERT INTO {$tableName} SET " . $this->genBackSql($arr, ', ');
       ...
       if($is==1){
           // echo "写入操作成功";
           interface_log(INFO, EC_OK, "*****写入数据库成功:{$str}*****");
           return 1;
        }
        else if($is==0)
        {
            //echo "操作失败";
            interface_log(INFO, EC_OK, "***** 写入数据库失败:{$str}*****");
            return 0;
        }else if($is==2)
        {
           // echo "有行受影响";
           interface_log(INFO, EC_OK, "*****没有数据写入数据库:{$str}*****");
           return 2;
        }
    ...
    function getCount(array $args = array()) {
        $args['_field'] = 'COUNT(*) as conut';
        return $this->getObject($args)[0]['conut'];
    }        
    ...
    function delObject($where=array()) {
       ...
       if($is==1){
           // echo "写入操作成功";
           interface_log(INFO, EC_OK, "*****操作数据库成功:{$str}*****");
        }
        else if($is==0)
        {
            //echo "操作失败";
            interface_log(INFO, EC_OK, "***** 操作数据库失败:{$str}*****");
        }else if($is==2)
        {
           // echo "有行受影响";
